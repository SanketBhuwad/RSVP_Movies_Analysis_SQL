...
SELECT 
    TABLE_NAME AS 'Table\'s', TABLE_ROWS AS 'Count of Rows'
FROM
...
WITH genre_counter
     AS (SELECT movie_id , Count(genre) Count_genre
         FROM   genre
         GROUP  BY movie_id)
SELECT Count(movie_id) Single_genre_Movies
FROM   genre_counter
WHERE  count_genre = 1; 
...
(
           SELECT     d.name_id, NAME, r.avg_rating, total_votes,
                      m.date_published,d.movie_id,  duration,
